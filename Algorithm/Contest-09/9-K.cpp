...
int Dinic(long long start, long long end) {
...
void Task() {
  ...
  for (long long index = 0; index < num_edges; ++index) {
    ...
    edges.push_back(Edge(edge.from, edge.to, edge.capacity));
    graph[edge.to].push_back(edges.size());
    edges.push_back(Edge(edge.to, edge.from, 0));
  }
  std::cout << Dinic(start, end);
}
