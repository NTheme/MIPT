...
void testRational() {
  ...
  BI b = 15;
  (r /= 8) -= b;  // r = 7-15=-8
  if (-r != 8) throw std::runtime_error("Test 1 failed.");
  ...
  if (q / 1000000000 >= 1) throw std::runtime_error("Test 4 failed.");
  if (0 / q != 0) throw std::runtime_error("Test 5 failed.");
  q *= t / s;
  if (q != 1 || q.toString() != "1") throw std::runtime_error("Test 6 failed.");
