...
bool equals(double a, double b) { return a - b <= 1e-6 && b - a <= 1e-6; }
...
int testGeometry() {
  ...
  if (!equals(9 * abfced.area(), newAbfced.area())) {
    std::cerr << "Test 8 failed. (after rotation and scaling by k, area must "
                 "increase in k^2 times)\n";
    return 1;
  }
  if (!equals(3 * abfced.perimeter(), newAbfced.perimeter())) {
    std::cerr << "Test 9 failed. (after rotation and scaling by k, perimeter "
                 "must increase also in k times)\n";
    return 1;
  }
  ...
  if (!abfced.isCongruentTo(newAbfced)) {
    std::cerr
        << "Test 11 failed. (transformations or congruency of polygons)\n";
    return 1;
  }
  if (newAbfced == abfced) {
  ...
  if (!(rec_ae1_shape == sq_ae_shape) || rec_ae1_shape != sq_ae_shape) {
    std::cerr << "Test 12.5 failed. (Square and rectangle may be equal, even "
                 "if their static type is Shape)\n";
    return 1;
  }
  Circle b3(b, 3);
  Ellipse cf5(c, f, 5);
  ...
  std::cerr << "Tests Log: Test 15 completed, now just scale shapes "
               "(without any checks)\n";
  ...
  // Ellipse testing
  cf5 = Ellipse(f, c, 5);
  // std::cerr << "Tests Log: Now there must be only one Ellipse, with focuses
  // (3;-1),(6;1) and dist 5\n";
  {
    double c = sqrt(13) / 2;
    double a = 5. / 2;
    ...
    if (!equals(cf5.perimeter(), per)) {
      std::cerr << "Test 16.1 failed. (ellipse perimeter) CORRECT: " << per << " ANS: " << cf5.perimeter() << '\n';
      return 1;
    }
  ...
  abd = Triangle(d, b, a);
  {
    ...
    if (!ok) {
      std::cerr << "Test 17.2 failed. (radius of the nine-point circle is half "
                   "of the circumcircle radius)\n";
      return 1;
    }
    ...
    if (!ok) {
      std::cerr << "Test 17.3 failed. (homothety of nine-points circle wrt "
                   "orthocenter with k=2 gives circumcircle)\n";
      return 1;
    }
    ...
    if (!ok) {
      std::cerr << "Test 17.4 failed. (centroid and nine-points circle center "
                   "lie on Euler line)\n";
      return 1;
    }
  }
