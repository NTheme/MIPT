...
template <template <typename, typename> class Alloccc>
void TestPerformance() {
...

  {
    ...
    first = ListPerformanceTest(std::list<int, std::allocator<int>>());
    second = ListPerformanceTest(
        std::list<int, StackAllocator<int, STORAGE_SIZE>>(alloc));
    std::ignore = first;
    std::ignore = second;
    first = 0, second = 0;
  }
